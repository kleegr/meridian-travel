...
export default function Financials({ itineraries }: { itineraries: Itinerary[] }) {
  ...
  const agentProfit = AGENTS.map((a) => ({
    name: a,
    profit: allFin.filter((i) => i.agent === a).reduce((x, y) => x + y.fin.profit, 0),
  }))
    .filter((a) => a.profit > 0)
  ...
  return (
    <div className="space-y-7">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-1">Financials</h2>
        <p className="text-gray-400 text-sm">Global revenue & profit overview \u00b7 {filtered.length} of {itineraries.length} itineraries</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div>
            ...
            <select value={filterDest} onChange={(e) => setFilterDest(e.target.value)} className={sel}>
              <option value="All">All</option>
              {destinations.map((d) => <option key={d}>{d}</option>)}
            </select>
          </div>
          <button onClick={clearFilters} className="px-3 py-2 text-sm font-medium text-gray-500 hover:bg-gray-100 rounded-lg transition-colors">Clear</button>
        </div>
      </div>
      ...
      {/* Agent profit */}
      {agentProfit.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
          <h3 className="font-semibold text-gray-800 mb-4 text-sm uppercase tracking-wider">Profit by Agent</h3>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {agentProfit.map((a) => (
              <div key={a.name} className="text-center p-3 rounded-lg bg-gray-50">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white mx-auto mb-2" style={{ background: GHL.accent }}>
                  {a.name.split(' ').map((n) => n[0]).join('')}
                </div>
                <p className="text-xs text-gray-500">{a.name}</p>
                <p className="font-bold text-sm" style={{ color: GHL.success }}>{fmt(a.profit)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h3 className="font-semibold text-gray-800">Itinerary P&L</h3>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr style={{ background: '#f8fafc' }} className="border-b border-gray-100">
              {['Itinerary', 'Client', 'Agent', 'Dates', 'Status', 'Revenue', 'Cost', 'Profit', 'Margin'].map((h) => (
          ...
          <tbody className="divide-y divide-gray-50">
            {allFin.map((i) => (
              <tr key={i.id} className="hover:bg-gray-50/50">
                <td className="px-5 py-4 font-medium text-gray-900">{i.title}</td>
                <td className="px-5 py-4 text-gray-600">{i.client}</td>
                <td className="px-5 py-4 text-gray-600">{i.agent}</td>
                <td className="px-5 py-4 text-gray-500 text-xs">{fmtDate(i.startDate)}</td>
                <td className="px-5 py-4"><StatusBadge status={i.status} /></td>
                <td className="px-5 py-4 font-medium text-gray-800">{fmt(i.fin.totalSell)}</td>
                <td className="px-5 py-4 text-gray-600">{fmt(i.fin.totalCost)}</td>
                <td className="px-5 py-4 font-semibold" style={{ color: GHL.success }}>{fmt(i.fin.profit)}</td>
                <td className="px-5 py-4 text-gray-500">{i.fin.margin}%</td>
              </tr>
            ))}
          </tbody>
        </table>
