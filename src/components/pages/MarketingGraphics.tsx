...
export default function MarketingGraphics({ packages, agencyProfile }: Props) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const svgBlob = new Blob([`<svg xmlns="http://www.w3.org/2000/svg" width="${s.w}" height="${s.h}"><foreignObject width="100%" height="100%">${data}</foreignObject></svg>`], { type: 'image/svg+xml' });
      ...
      alert('Use right-click > Save Image, or take a screenshot of the preview.');
    }
  };
  ...
  const renderPreview = () => {
    ...
    return (
      <div ref={previewRef} id="marketing-preview" style={{ width: pw, height: ph, background: uploadedImage ? `url(${uploadedImage}) center/cover no-repeat` : t.bg, borderRadius: isFlyer ? 0 : 16, overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: pad, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: showContact ? 10 : 0 }}>
              ...
              {showCta && (<span style={{ background: uploadedImage ? '#ffffff' : t.accent, color: uploadedImage ? '#000' : (t.id === 'modern' ? '#fff' : '#000'), padding: '6px 16px', borderRadius: 20, fontSize: 10, fontWeight: 700 }}>{ctaText}</span>)}
            </div>
            {showContact && (<div style={{ borderTop: `1px solid ${uploadedImage ? 'rgba(255,255,255,0.2)' : (t.accent + '30')}`, paddingTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: isFlyer ? 16 : 10, flexWrap: 'wrap' }}>{agencyProfile.phone && <span style={{ color: uploadedImage ? 'rgba(255,255,255,0.7)' : t.textColor, fontSize: 8, opacity: uploadedImage ? 1 : 0.5 }}>{agencyProfile.phone}</span>}{agencyProfile.email && <span style={{ color: uploadedImage ? 'rgba(255,255,255,0.7)' : t.textColor, fontSize: 8, opacity: uploadedImage ? 1 : 0.5 }}>{agencyProfile.email}</span>}{agencyProfile.address && <span style={{ color: uploadedImage ? 'rgba(255,255,255,0.7)' : t.textColor, fontSize: 8, opacity: uploadedImage ? 1 : 0.5 }}>{agencyProfile.address}</span>}</div>)}
          </div>
        </div>
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-white rounded-xl border p-4 shadow-sm" style={{ borderColor: GHL.border }}>
            ...
            <div className="flex items-center gap-3">
              {uploadedImage ? (<div className="w-16 h-16 rounded-lg overflow-hidden border flex-shrink-0" style={{ borderColor: GHL.border }}><img src={uploadedImage} alt="" className="w-full h-full object-cover" /></div>) : (<div className="w-16 h-16 rounded-lg border-2 border-dashed flex items-center justify-center cursor-pointer hover:border-blue-300" style={{ borderColor: GHL.border }} onClick={() => fileRef.current?.click()}><Icon n="plus" c="w-5 h-5" /></div>)}
              <div><button onClick={() => fileRef.current?.click()} className="text-xs font-medium px-3 py-1.5 rounded-lg border hover:bg-gray-50" style={{ borderColor: GHL.border, color: GHL.accent }}>{uploadedImage ? 'Change' : 'Upload Photo'}</button>{uploadedImage && <button onClick={() => setUploadedImage(null)} className="text-xs ml-2 text-red-400 hover:text-red-600">Remove</button>}<p className="text-[9px] mt-1" style={{ color: GHL.muted }}>JPG, PNG. Max 5MB.</p></div>
            </div>
            <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
          </div>
          {!uploadedImage && (<div className="bg-white rounded-xl border p-4 shadow-sm" style={{ borderColor: GHL.border }}><p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: GHL.muted }}>Style</p><div className="grid grid-cols-3 gap-2">{TEMPLATES.map((t) => (<button key={t.id} onClick={() => setSelectedTemplate(t)} className="rounded-lg p-2 text-center transition-all" style={{ background: selectedTemplate.id === t.id ? GHL.accentLight : GHL.bg, border: selectedTemplate.id === t.id ? `2px solid ${GHL.accent}` : '2px solid transparent' }}><div style={{ width: '100%', height: 24, borderRadius: 4, background: t.bg }} /><p className="text-[9px] mt-1" style={{ color: GHL.text }}>{t.name}</p></button>))}</div></div>)}
          <div className="bg-white rounded-xl border p-4 shadow-sm" style={{ borderColor: GHL.border }}>
          ...
          <div className="bg-white rounded-xl border p-4 shadow-sm" style={{ borderColor: GHL.border }}>
            ...
            {showCta && (<div className="space-y-2 ml-6"><input value={ctaText} onChange={(e) => setCtaText(e.target.value)} placeholder="Button text (e.g. Book Now)" className="w-full px-3 py-2 border rounded-lg text-sm" style={{ borderColor: GHL.border }} /><input value={ctaLink} onChange={(e) => setCtaLink(e.target.value)} placeholder="Link URL (e.g. https://yoursite.com/book)" className="w-full px-3 py-2 border rounded-lg text-sm" style={{ borderColor: GHL.border }} /></div>)}
          ...
          <div className="bg-white rounded-xl border p-6 shadow-sm" style={{ borderColor: GHL.border }}>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={handleDownload} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white" style={{ background: GHL.accent }}><Icon n="print" c="w-3 h-3" /> Download</button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mt-3">
              <p className="text-xs" style={{ color: GHL.muted }}>Right-click preview to save, or click Download</p>
