...
import { GHL, AGENTS, STATUSES, DEFAULT_CHECKLIST } from '@/lib/constants';
...
import type { Itinerary, Flight, Hotel, Transport, Attraction, Insurance, CarRental, Passenger, Davening, Mikvah, Row, AgencyProfile, FormField, Pipeline } from '@/lib/types';

interface Props { itin: Itinerary; onBack: () => void; onUpdate: (u: Itinerary) => void; onDelete?: () => void; agencyProfile: AgencyProfile; pipelines?: Pipeline[]; }

// Helper: convert any booking item to Record<string, string> for form
function toFormData(item: any): Record<string, string> {
  const d: Record<string, string> = {};
  Object.entries(item).forEach(([k, v]) => { if (v !== null && v !== undefined) d[k] = String(v); });
  return d;
}

export default function ItineraryDetail({ itin, onBack, onUpdate, onDelete, agencyProfile, pipelines }: Props) {
  ...
  const allStages = pipelines?.flatMap((p) => p.stages) || STATUSES;
  ...
  const handleAdd = (section: string, data: Record<string, string>) => { const entry = { ...data, id: uid(), cost: parseFloat(data.cost) || 0, sell: parseFloat(data.sell) || 0 }; const u = { ...itin }; switch (section) { case 'flight': u.flights = [...itin.flights, entry as unknown as Flight]; break; case 'hotel': u.hotels = [...itin.hotels, { ...entry, rooms: parseInt(data.rooms) || 1 } as unknown as Hotel]; break; case 'transport': u.transport = [...itin.transport, entry as unknown as Transport]; break; case 'attraction': u.attractions = [...itin.attractions, entry as unknown as Attraction]; break; case 'insurance': u.insurance = [...itin.insurance, entry as unknown as Insurance]; break; case 'carRental': u.carRentals = [...itin.carRentals, entry as unknown as CarRental]; break; case 'passenger': u.passengerList = [...itin.passengerList, { ...entry, id: uid() } as unknown as Passenger]; u.passengers = u.passengerList.length; break; case 'davening': u.davening = [...(itin.davening||[]), { ...entry, id: uid() } as unknown as Davening]; break; case 'mikvah': u.mikvah = [...(itin.mikvah||[]), { ...entry, id: uid() } as unknown as Mikvah]; break; } onUpdate(u); setAddModal(null); };

  const handleAddMultiFlights = (flights: Record<string, string>[]) => { onUpdate({ ...itin, flights: [...itin.flights, ...flights.map((d) => ({ ...d, id: uid(), cost: parseFloat(d.cost) || 0, sell: parseFloat(d.sell) || 0 } as unknown as Flight))] }); };
  const handleFlightSave = (data: Record<string, string>) => { onUpdate({ ...itin, flights: [...itin.flights, { ...data, id: uid(), cost: parseFloat(data.cost) || 0, sell: parseFloat(data.sell) || 0 } as unknown as Flight] }); setAddModal(null); };

  // EDIT: save edited booking item
  const handleEditItemSave = (data: Record<string, string>) => {
    if (!editItem) return;
    const { section, id } = editItem;
    const merged = { ...data, id, cost: parseFloat(data.cost) || 0, sell: parseFloat(data.sell) || 0 };
    const u = { ...itin };
    switch (section) {
      case 'flight': u.flights = itin.flights.map((f) => f.id === id ? { ...f, ...merged } as unknown as Flight : f); break;
      case 'hotel': u.hotels = itin.hotels.map((h) => h.id === id ? { ...h, ...merged, rooms: parseInt(data.rooms) || 1 } as unknown as Hotel : h); break;
      case 'transport': u.transport = itin.transport.map((t) => t.id === id ? { ...t, ...merged } as unknown as Transport : t); break;
      case 'attraction': u.attractions = itin.attractions.map((a) => a.id === id ? { ...a, ...merged } as unknown as Attraction : a); break;
      case 'insurance': u.insurance = itin.insurance.map((x) => x.id === id ? { ...x, ...merged } as unknown as Insurance : x); break;
      case 'carRental': u.carRentals = itin.carRentals.map((c) => c.id === id ? { ...c, ...merged } as unknown as CarRental : c); break;
      case 'davening': u.davening = (itin.davening||[]).map((d) => d.id === id ? { ...d, ...data, id } as unknown as Davening : d); break;
      case 'mikvah': u.mikvah = (itin.mikvah||[]).map((m) => m.id === id ? { ...m, ...data, id } as unknown as Mikvah : m); break;
    }
    onUpdate(u);
    setEditItem(null);
  };

  // Get the item being edited + its fields
  const getEditData = (): { fields: FormField[]; initial: Record<string, string>; title: string; mode?: 'flight' | 'hotel' | 'default' } | null => {
    if (!editItem) return null;
    const { section, id } = editItem;
    switch (section) {
      case 'flight': { const f = itin.flights.find((x) => x.id === id); return f ? { fields: FLIGHT_FIELDS, initial: toFormData(f), title: 'Edit Flight', mode: 'flight' } : null; }
      case 'hotel': { const h = itin.hotels.find((x) => x.id === id); return h ? { fields: HOTEL_FIELDS, initial: toFormData(h), title: 'Edit Hotel', mode: 'hotel' } : null; }
      case 'transport': { const t = itin.transport.find((x) => x.id === id); return t ? { fields: TRANSPORT_FIELDS, initial: toFormData(t), title: 'Edit Transfer' } : null; }
      case 'attraction': { const a = itin.attractions.find((x) => x.id === id); return a ? { fields: ATTRACTION_FIELDS, initial: toFormData(a), title: 'Edit Activity' } : null; }
      case 'insurance': { const x = itin.insurance.find((x) => x.id === id); return x ? { fields: INSURANCE_FIELDS, initial: toFormData(x), title: 'Edit Insurance' } : null; }
      case 'carRental': { const c = itin.carRentals.find((x) => x.id === id); return c ? { fields: CAR_RENTAL_FIELDS, initial: toFormData(c), title: 'Edit Car Rental' } : null; }
      case 'davening': { const d = (itin.davening||[]).find((x) => x.id === id); return d ? { fields: DAVENING_FIELDS, initial: toFormData(d), title: 'Edit Davening' } : null; }
      case 'mikvah': { const m = (itin.mikvah||[]).find((x) => x.id === id); return m ? { fields: MIKVAH_FIELDS, initial: toFormData(m), title: 'Edit Mikvah' } : null; }
    }
    return null;
  };
  ...
  const resetChecklist = () => { if (confirm('Reset checklist to defaults?')) { const cl = DEFAULT_CHECKLIST.map((c) => ({ ...c })); if (itin.isVip) cl.push({ id: uid(), text: 'Send VIP welcome gift', done: false }); onUpdate({ ...itin, checklist: cl }); } };

  const pR = (r: Row) => <span style={{ color: GHL.success }} className="font-semibold">{fmt(Number(r.sell) - Number(r.cost))}</span>;
  const cR = (r: Row) => fmt(Number(r.cost));
  const sR = (r: Row) => fmt(Number(r.sell));
  const checkDone = itin.checklist.filter((c) => c.done).length;
  const checkTotal = itin.checklist.length || 1;
  const diCount = (itin.destinationInfo || []).length;
  const editFields: FormField[] = ITINERARY_FIELDS.map((f) => { if (f.key === 'agent') return { ...f, options: AGENTS }; if (f.key === 'status') return { ...f, options: uniqueStages }; return f; });
  const editInit = { title: itin.title, client: itin.client, agent: itin.agent, destination: itin.destination, startDate: itin.startDate, endDate: itin.endDate, passengers: String(itin.passengers), status: itin.status, tags: itin.tags.join(', '), notes: itin.notes, isVip: itin.isVip ? 'true' : '' };
  const ed = getEditData();

  return (
    <div className="space-y-5">
      <div className="flex items-center gap-4 flex-wrap">
        <button onClick={onBack} className="p-2 rounded-lg hover:bg-gray-100" style={{ color: GHL.muted }}><Icon n="back" c="w-5 h-5" /></button>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap"><h2 className="text-2xl font-bold truncate" style={{ color: GHL.text }}>{itin.title}</h2><StatusBadge status={itin.status} />{itin.isVip && <span className="text-[10px] font-bold px-2 py-0.5 rounded" style={{ background: '#fef3c7', color: '#d97706', border: '1px solid #fde68a' }}>VIP</span>}{itin.tags.map((t) => <span key={t} className="text-xs px-2 py-1 rounded-full" style={{ background: GHL.accentLight, color: GHL.accent }}>{t}</span>)}</div>
          <p className="text-sm mt-0.5" style={{ color: GHL.muted }}>{itin.client} &middot; {itin.agent} &middot; {(itin.destinations?.length > 1) ? itin.destinations.join(', ') : itin.destination}</p>
        </div>
        <div className="flex gap-2 flex-shrink-0">
          <button onClick={() => setEditModal(true)} className="p-2.5 rounded-lg border hover:bg-gray-50" style={{ borderColor: GHL.border, color: GHL.muted }}><Icon n="edit" c="w-4 h-4" /></button>
          <button onClick={handleDuplicate} className="p-2.5 rounded-lg border hover:bg-gray-50" style={{ borderColor: GHL.border, color: GHL.muted }}><Icon n="copy" c="w-4 h-4" /></button>
          {onDelete && <button onClick={() => { if (confirm('Delete?')) onDelete(); }} className="p-2.5 rounded-lg border hover:bg-red-50" style={{ borderColor: GHL.border, color: GHL.muted }}><Icon n="trash" c="w-4 h-4" /></button>}
      ...
      {tab === 'bookings' && <div className="space-y-4">
        ...
        <Accordion title="Mikvah" icon="globe" count={(itin.mikvah||[]).length} onAdd={() => setAddModal('mikvah')}><MiniTable cols={[{ key: 'name', label: 'Mikvah' }, { key: 'city', label: 'Area' }, { key: 'address', label: 'Address' }, { key: 'hours', label: 'Hours' }]} rows={(itin.mikvah||[]) as unknown as Row[]} addLabel="Add" onAdd={() => setAddModal('mikvah')} onEdit={(id) => setEditItem({ section: 'mikvah', id })} onDelete={(id) => handleDel('mikvah', id)} /></Accordion>
      </div>}

      {tab === 'destinations' && <DestinationInfoSection itin={itin} onUpdate={onUpdate} />}

      {tab === 'checklist' && <div className="bg-white rounded-xl border p-6 shadow-sm" style={{ borderColor: GHL.border }}>
        <div className="flex items-center justify-between mb-4"><h3 className="font-semibold" style={{ color: GHL.text }}>Agent Checklist</h3><div className="flex items-center gap-2"><span className="text-sm font-semibold" style={{ color: checkDone === itin.checklist.length ? GHL.success : GHL.accent }}>{Math.round((checkDone / checkTotal) * 100)}%</span>{itin.checklist.length === 0 && <button onClick={resetChecklist} className="text-xs font-medium px-3 py-1.5 rounded-lg border" style={{ borderColor: GHL.border, color: GHL.accent }}>Reset to Defaults</button>}</div></div>
        <div className="h-2 rounded-full overflow-hidden mb-6" style={{ background: GHL.bg }}><div className="h-full rounded-full" style={{ width: `${Math.round((checkDone / checkTotal) * 100)}%`, background: checkDone === itin.checklist.length ? GHL.success : GHL.accent }} /></div>
        ...
        {itin.checklist.length > 0 && <button onClick={resetChecklist} className="mt-3 text-xs" style={{ color: GHL.muted }}>Reset to default checklist</button>}
      </div>}
      ...
      {/* Add modals */}
      {addModal === 'flight' && <SmartFormModal title="Add Flight" fields={FLIGHT_FIELDS} onSave={handleFlightSave} onClose={() => setAddModal(null)} mode="flight" onSaveMultipleFlights={handleAddMultiFlights} />}
      ...
      {/* Edit booking item modal */}
      {editItem && ed && (
        ed.mode === 'flight' || ed.mode === 'hotel'
          ? <SmartFormModal title={ed.title} fields={ed.fields} onSave={handleEditItemSave} onClose={() => setEditItem(null)} initial={ed.initial} mode={ed.mode} />
          : <FormModal title={ed.title} fields={ed.fields} onSave={handleEditItemSave} onClose={() => setEditItem(null)} initial={ed.initial} />
      )}

      {/* Edit itinerary modal */}
      {editModal && <SmartFormModal title="Edit Itinerary" fields={editFields} onSave={handleEditSave} onClose={() => setEditModal(false)} initial={editInit} />}
    </div>
  );
}
