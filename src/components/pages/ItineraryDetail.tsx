  ...
  return (
    <div className="space-y-5">
      ...
      <div className="border-b flex gap-1 overflow-x-auto" style={{ borderColor: GHL.border }}>
        {[{ id: 'overview', l: 'Overview' }, { id: 'passengers', l: 'Passengers', cnt: itin.passengerList.length }, { id: 'bookings', l: 'Bookings' }, { id: 'destinations', l: 'Dest. Info', cnt: di }, { id: 'checklist', l: 'Checklist', cnt: itin.checklist.length }, { id: 'financials', l: 'Financials' }, { id: 'print', l: 'Client Itinerary' }, { id: 'map', l: 'Map' }].map((t) => (
      ...
      {tab === 'bookings' && <div className="space-y-4">
        <Accordion title="Flights" icon="plane" count={itin.flights.length} defaultOpen onAdd={() => setAddModal('flight')}><MiniTable cols={[{ key: 'from', label: 'From' }, { key: 'to', label: 'To' }, { key: 'airline', label: 'Airline' }, { key: 'flightNo', label: '#' }, { key: 'status', label: 'Status' }, { key: 'duration', label: 'Dur.' }, { key: 'pnr', label: 'PNR' }, { key: 'cost', label: 'Cost', render: cR }, { key: 'sell', label: 'Sell', render: sR }, { key: 'profit', label: 'Profit', render: pR }]} rows={itin.flights as unknown as Row[]} addLabel="Add" onAdd={() => setAddModal('flight')} onEdit={(id) => setEditItem({ section: 'flight', id })} onDelete={(id) => handleDel('flight', id)} /></Accordion>
        ...
        <div className="space-y-4">
          {checklistTemplates.length > 0 && (
            <div className="bg-white rounded-xl border p-5 shadow-sm" style={{ borderColor: GHL.border }}>
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-bold uppercase tracking-wider" style={{ color: GHL.muted }}>Apply Checklist Template</p>
                {currentTpl && <span className="text-xs px-2 py-1 rounded-lg" style={{ background: GHL.accentLight, color: GHL.accent }}>Current: {currentTpl.name}</span>}
              </div>
          ...
          <div className="bg-white rounded-xl border p-6 shadow-sm" style={{ borderColor: GHL.border }}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold" style={{ color: GHL.text }}>Agent Checklist</h3>
              <span className="text-sm font-semibold" style={{ color: ck === itin.checklist.length ? GHL.success : GHL.accent }}>{Math.round((ck / ct) * 100)}%</span>
            </div>
            <div className="h-2 rounded-full overflow-hidden mb-6" style={{ background: GHL.bg }}>
              <div className="h-full rounded-full" style={{ width: `${Math.round((ck / ct) * 100)}%`, background: ck === itin.checklist.length ? GHL.success : GHL.accent }} />
            </div>
            <div className="space-y-1">
              {itin.checklist.map((c) => {
                const nc = (c.notes || []).length;
                ...
                return (
                  <div key={c.id} className="rounded-lg border" style={{ borderColor: isOpen ? GHL.border : 'transparent' }}>
                    <div className="flex items-center gap-3 py-2 px-3 hover:bg-gray-50 group">
                      ...
                      {nc > 0 && !isOpen && <button onClick={() => setExpandedCheckId(c.id)} className="inline-flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.5 rounded" style={{ background: '#dbeafe', color: '#1e40af' }}><Icon n="message" c="w-2.5 h-2.5" />{nc}</button>}
                      <button onClick={() => setExpandedCheckId(isOpen ? null : c.id)} className={`p-1 rounded transition-colors ${isOpen ? '' : 'opacity-0 group-hover:opacity-100'}`} style={{ color: GHL.muted }}><Icon n="message" c="w-3.5 h-3.5" /></button>
            ...
            <div className="flex gap-2 mt-4 pt-4 border-t" style={{ borderColor: GHL.border }}>
              <input value={newCheckItem} onChange={(e) => setNewCheckItem(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addCheckItem()} placeholder="Add item..." className="flex-1 px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-200" style={{ borderColor: GHL.border }} />
