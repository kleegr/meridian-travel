...
export default function Dashboard({ itineraries, widgets, onToggleWidget, onSelectItinerary }: Props) {
  ...
  const recentTrips = useMemo(() => [...itineraries].sort((a, b) => new Date(b.created).getTime() - new Date(a.created).getTime()).slice(0, 5), [itineraries]);
    ...
    <div className="space-y-5">
      ...
      {w('overview') && <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[{ label: 'Total Trips', value: stats.total, icon: 'map', color: GHL.accent, bg: GHL.accentLight },
          ...
          { label: 'VIP', value: stats.vip, icon: 'star', color: '#d97706', bg: '#fef3c7' },
          { label: 'Upcoming', value: stats.upcoming, icon: 'calendar', color: '#7c3aed', bg: '#f3e8ff' },
        ...
        ].map(m => (<div key={m.label} className="bg-white rounded-xl border p-4 shadow-sm" style={{ borderColor: GHL.border }}><div className="flex items-center gap-2 mb-2"><span className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: m.bg, color: m.color }}><Icon n={m.icon} c="w-4 h-4" /></span></div><p className="text-xl font-bold" style={{ color: m.color }}>{m.value}</p><p className="text-[10px] font-medium uppercase tracking-wider mt-0.5" style={{ color: GHL.muted }}>{m.label}</p></div>))}
      </div>}
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-xl border p-5 shadow-sm" style={{ borderColor: GHL.border }}>
            ...
            <div className="space-y-1.5">{recentTrips.map(trip => { const meta = getStatusMeta(trip.status); const n = nights(trip.startDate, trip.endDate); return (<div key={trip.id} onClick={() => onSelectItinerary?.(trip.id)} className="flex items-center gap-3 p-3 rounded-lg hover:bg-blue-50/50 transition-colors cursor-pointer">
              ...
              <div className="flex-1 min-w-0"><p className="text-sm font-semibold truncate" style={{ color: GHL.text }}>{trip.title}</p><p className="text-[10px]" style={{ color: GHL.muted }}>{trip.client} {String.fromCharCode(8226)} {trip.destination} {String.fromCharCode(8226)} {n} nights</p></div>
            ...
            </div>); })}{recentTrips.length === 0 && <p className="text-sm text-center py-4" style={{ color: GHL.muted }}>No itineraries yet</p>}</div>
          </div>
        </div>
