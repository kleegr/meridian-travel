  ...
  return (
    <div className="space-y-7">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowCustomize(!showCustomize)} className="inline-flex items-center gap-2 text-sm font-medium px-3 py-2 rounded-lg border border-gray-200 hover:bg-gray-50 text-gray-600 transition-colors"><Icon n="settings" c="w-4 h-4" />{showCustomize ? 'Done' : 'Customize'}</button>
      </div>

      {showCustomize && (
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          <h3 className="font-semibold text-gray-800 mb-1 text-sm">Toggle Dashboard Widgets</h3>
          <p className="text-xs text-gray-400 mb-3">Show or hide any section on your dashboard</p>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {widgets.map((wg) => (
              <button key={wg.id} onClick={() => onToggleWidget(wg.id)} className="flex items-center gap-2 px-3 py-2.5 rounded-lg border text-sm font-medium transition-colors" style={wg.enabled ? { background: GHL.accentLight, borderColor: GHL.accent, color: GHL.accent } : { background: '#f3f4f6', borderColor: '#e5e7eb', color: '#9ca3af' }}>
                <span className="w-4 h-4 rounded border flex items-center justify-center" style={wg.enabled ? { background: GHL.accent, borderColor: GHL.accent } : { borderColor: '#d1d5db' }}>{wg.enabled && <Icon n="check" c="w-3 h-3 text-white" />}</span>
                {wg.label}
              </button>
            ))}
          </div>
      ...
      {w('financials') && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="rounded-xl p-5 text-white shadow-sm" style={{ background: GHL.sidebar }}><p className="text-xs uppercase tracking-wider mb-2 opacity-70">Revenue</p><p className="text-xl font-bold">{fmt(totalRev)}</p></div>
          <div className="rounded-xl p-5 text-white shadow-sm" style={{ background: GHL.accent }}><p className="text-xs uppercase tracking-wider mb-2 opacity-70">Profit</p><p className="text-xl font-bold">{fmt(totalProfit)}</p></div>
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm"><p className="text-xs text-gray-400 uppercase tracking-wider mb-2">Outstanding</p><p className="text-xl font-bold" style={{ color: GHL.warning }}>{fmt(allFin.reduce((a, b) => a + b.fin.balance, 0))}</p></div>
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm"><p className="text-xs text-gray-400 uppercase tracking-wider mb-2">Avg Per Trip</p><p className="text-xl font-bold" style={{ color: GHL.accent }}>{fmt(totalProfit / (itineraries.length || 1))}</p></div>
        </div>
      )}
      ...
      {w('checklist') && (
        <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
          ...
          <div className="space-y-2">{itineraries.filter((i) => i.status !== 'Cancelled' && i.status !== 'Completed').slice(0, 5).map((i) => { const done = i.checklist.filter((c) => c.done).length; const total = i.checklist.length || 1; const pct = Math.round((done / total) * 100); return (<div key={i.id} className="flex items-center gap-3"><div className="flex-1"><div className="flex justify-between mb-1"><span className="text-sm font-medium text-gray-700">{i.title}</span><span className="text-xs text-gray-400">{done}/{total}</span></div><div className="h-1.5 bg-gray-100 rounded-full overflow-hidden"><div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: pct === 100 ? GHL.success : GHL.accent }} /></div></div></div>); })}</div>
        </div>
      )}
    </div>
  );
