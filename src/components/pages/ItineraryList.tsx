...
export default function ItineraryList({ itineraries, statusLabels, onSelect, onCreate, onUpdateStatus }: ItineraryListProps) {
  ...
  const sel = 'px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 bg-white text-gray-700';

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h2 className="text-2xl font-bold text-gray-900 mb-1">Itineraries</h2><p className="text-gray-400 text-sm">{itineraries.length} total &middot; {filtered.length} shown</p></div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px]"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><Icon n="search" c="w-4 h-4" /></span><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search trips, clients, travelers..." className="w-full pl-9 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20" /></div>
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className={sel}><option value="All">All Statuses</option>{statusLabels.map((s) => <option key={s}>{s}</option>)}</select>
