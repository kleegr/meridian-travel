...
const DEFAULT_CARD_CONFIG: CardViewConfig = { showProfit: true, showChecklist: true, showAgent: true, showDate: true, showDestination: true, showPax: true, showVip: true };
...
export default function ItineraryList({ itineraries, pipelines, activePipelineId, onSetActivePipeline, onSelect, onCreate, onUpdateStatus, onDelete }: ItineraryListProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {view === 'list' && <div className="bg-white rounded-xl border shadow-sm overflow-hidden" style={{ borderColor: GHL.border }}><table className="w-full text-sm"><thead><tr style={{ background: GHL.bg }}>{['Trip / Client', 'Destination', 'Agent', 'Dates', 'Pax', 'Stage', 'Revenue', 'Profit', ''].map((h) => <th key={h} className="text-left px-5 py-4 text-xs font-semibold uppercase tracking-wider" style={{ color: GHL.muted }}>{h}</th>)}</tr></thead><tbody className="divide-y">{filtered.map((i) => { const fin = calcFin(i); return (<tr key={i.id} className="hover:bg-blue-50/30 transition-colors"><td className="px-5 py-4 cursor-pointer" onClick={() => onSelect(i.id)}><div className="flex items-center gap-1.5"><p className="font-semibold" style={{ color: GHL.text }}>{i.title}</p>{i.isVip && <span className="text-[9px] font-bold px-1 py-0.5 rounded" style={{ background: '#fef3c7', color: '#d97706' }}>VIP</span>}</div><p className="text-xs mt-0.5" style={{ color: GHL.muted }}>{i.client}</p></td><td className="px-5 py-4 cursor-pointer" onClick={() => onSelect(i.id)} style={{ color: GHL.text }}>{(i.destinations && i.destinations.length > 1) ? i.destinations.join(', ') : i.destination}</td><td className="px-5 py-4" style={{ color: GHL.text }}>{i.agent}</td><td className="px-5 py-4 text-xs" style={{ color: GHL.muted }}>{fmtDate(i.startDate)}<br />{fmtDate(i.endDate)}</td><td className="px-5 py-4" style={{ color: GHL.text }}>{i.passengers}</td><td className="px-5 py-4"><StatusBadge status={i.status} /></td><td className="px-5 py-4 font-medium" style={{ color: GHL.text }}>{fmt(fin.totalSell)}</td><td className="px-5 py-4 font-semibold" style={{ color: GHL.success }}>{fmt(fin.profit)}</td><td className="px-5 py-4"><button onClick={() => { if (confirm('Delete this itinerary?')) onDelete(i.id); }} className="p-1.5 rounded hover:bg-red-50 text-gray-300 hover:text-red-500 transition-colors"><Icon n="trash" c="w-4 h-4" /></button></td></tr>); })}</tbody></table></div>}

      {view === 'grid' && <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">{filtered.map((i) => { const fin = calcFin(i); const m = STATUS_META[i.status]; const done = i.checklist.filter((c) => c.done).length; const allDone = done === i.checklist.length && i.checklist.length > 0; return (<div key={i.id} onClick={() => onSelect(i.id)} className="bg-white rounded-xl border shadow-sm p-5 cursor-pointer hover:shadow-md transition-all relative" style={{ borderColor: GHL.border, borderTop: `3px solid ${m?.dot || GHL.accent}` }}>{i.isVip && <span className="absolute top-2 right-2 text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ background: '#fef3c7', color: '#d97706', border: '1px solid #fde68a' }}>VIP</span>}<div className="flex items-start justify-between mb-3"><div><h3 className="font-bold" style={{ color: GHL.text }}>{i.title}</h3><p className="text-xs mt-0.5" style={{ color: GHL.muted }}>{i.client}</p></div><StatusBadge status={i.status} /></div><div className="flex items-center gap-2 text-xs mb-2" style={{ color: GHL.muted }}><Icon n="globe" c="w-3.5 h-3.5" />{(i.destinations && i.destinations.length > 1) ? i.destinations.join(', ') : i.destination} &middot; {i.passengers} pax</div><div className="h-1 rounded-full overflow-hidden mb-1" style={{ background: GHL.bg }}><div className="h-full rounded-full" style={{ width: `${Math.round((done / (i.checklist.length || 1)) * 100)}%`, background: allDone ? GHL.success : GHL.accent }} /></div><div className="flex items-center justify-between mb-3"><span className="text-[10px]" style={{ color: allDone ? GHL.success : GHL.muted }}>{done}/{i.checklist.length}</span>{!allDone && i.checklist.length > 0 && <span className="text-[9px] font-semibold" style={{ color: '#ef4444' }}>List Not Done</span>}</div><div className="border-t pt-3 flex justify-between" style={{ borderColor: GHL.border }}><div><p className="text-xs" style={{ color: GHL.muted }}>Revenue</p><p className="font-semibold text-sm" style={{ color: GHL.text }}>{fmt(fin.totalSell)}</p></div><div className="text-right"><p className="text-xs" style={{ color: GHL.muted }}>Profit</p><p className="font-semibold text-sm" style={{ color: GHL.success }}>{fmt(fin.profit)}</p></div></div><button onClick={(e) => { e.stopPropagation(); if (confirm('Delete this itinerary?')) onDelete(i.id); }} className="absolute bottom-3 right-3 p-1 rounded hover:bg-red-50 text-gray-200 hover:text-red-500 transition-colors"><Icon n="trash" c="w-3.5 h-3.5" /></button></div>); })}</div>}
    </div>
  );
}
