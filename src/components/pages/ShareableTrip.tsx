...
export default function ShareableTrip({ itin, agencyProfile, onBack }: Props) {
  ...
  const dests = (itin.destinations?.length > 1) ? itin.destinations.join(` ${dot} `) : itin.destination;
  ...
  if (itin.startDate && itin.endDate) {
    ...
    while (current <= end) {
      ...
      itin.flights.filter((f) => f.departure?.startsWith(dateStr)).forEach((f) => items.push({ type: 'flight', label: `${f.airline} ${f.flightNo}`, detail: `${f.from} ${arr} ${f.to}`, icon: 'plane', color: '#3b82f6' }));
      itin.hotels.filter((h) => h.checkIn === dateStr).forEach((h) => items.push({ type: 'hotel', label: h.name, detail: `Check-in ${dot} ${h.roomType || 'Room'}`, icon: 'hotel', color: '#f59e0b' }));
      ...
      itin.attractions.filter((a) => a.date === dateStr).forEach((a) => items.push({ type: 'activity', label: a.name, detail: `${a.city} ${dot} ${a.time || 'TBD'}`, icon: 'star', color: '#ec4899' }));
      itin.transport.filter((t) => t.pickupDateTime?.startsWith(dateStr)).forEach((t) => items.push({ type: 'transfer', label: `${t.type} - ${t.provider}`, detail: `${t.pickup} ${arr} ${t.dropoff}`, icon: 'car', color: '#8b5cf6' }));
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border shadow-lg overflow-hidden" style={{ borderColor: GHL.border }}>
            ...
            return (
              <div key={day.num} className="rounded-xl border overflow-hidden" style={{ borderColor: isOpen ? GHL.accent + '40' : GHL.border }}>
                <button onClick={() => setExpandedDay(isOpen ? null : day.num)} className="w-full flex items-center gap-3 p-4 text-left hover:bg-gray-50/50 transition-colors">
                  ...
                  <div className="flex-1"><p className="text-sm font-semibold" style={{ color: GHL.text }}>Day {day.num}</p><p className="text-[10px]" style={{ color: GHL.muted }}>{fmtDate(day.date)} {dot} {day.items.length} activities</p></div>
        ...
        <div className="px-6 py-4 border-t text-center" style={{ borderColor: GHL.border, background: GHL.bg }}>
          <p className="text-xs" style={{ color: GHL.muted }}>Prepared by {agencyProfile.name} {dot} {agencyProfile.email} {dot} {agencyProfile.phone}</p>
        </div>
      </div>
