...
function buildTL(it: Itinerary, cv: ClientViewSettings): TLE[] {
  ...
  if (cv.showHotels) {
    it.hotels.forEach((h) => {
      const ci = h.checkInTime || ''; const co = h.checkOutTime || '';
      let photos: string[] = [];
      try { if ((h as any).hotelPhotos) photos = typeof (h as any).hotelPhotos === 'string' ? JSON.parse((h as any).hotelPhotos) : (h as any).hotelPhotos; } catch {}
      e.push({ date: h.checkIn, sk: h.checkIn + ' ' + (ci ? '14:00' : '23:59'), type: 'Check-in', title: h.name, sub: `${h.city} \u00b7 ${h.roomType} \u00b7 ${h.rooms} room${h.rooms > 1 ? 's' : ''}`, detail: h.ref ? `Ref: ${h.ref}` : '', time: ci, noTime: !ci, city: h.city, section: 'hotel', itemId: h.id, hotelPhotos: photos });
      e.push({ date: h.checkOut, sk: h.checkOut + ' ' + (co ? '11:00' : '00:01'), type: 'Check-out', title: h.name, sub: h.city, time: co, noTime: !co, city: h.city, section: 'hotel', itemId: h.id });
    });
  }
  if (cv.showTransfers) {
...
const FONT_MAP: Record<string, string> = {
  ...
  'modern': "'SF Pro Display', 'Segoe UI', sans-serif",
};
...
export default function PrintView({ itin, agencyProfile, onEditItem }: Props) {
  ...
  const primaryColor = cv.primaryColor || '#093168';
  const accentColor = cv.accentColor || '#1a5298';
  const fontFamily = FONT_MAP[cv.fontFamily] || FONT_MAP['serif'];
  ...
  return (
    <div>
      <div className="no-print flex items-center justify-between px-6 py-3 mb-4 bg-white rounded-xl border shadow-sm" style={{ borderColor: '#D0E2FA' }}>
        ...
        <div className="flex items-center gap-3">{canEdit && <p className="text-xs" style={{ color: '#8599B4' }}>Click any item to edit</p>}<button onClick={() => window.print()} className="inline-flex items-center gap-2 text-white rounded-lg px-5 py-2.5 text-sm font-semibold hover:opacity-90 shadow-md" style={{ background: primaryColor }}>Print / Export PDF</button></div>
      </div>

      <div className="bg-white rounded-xl overflow-hidden shadow-lg" style={{ fontFamily }}>
        {/* COVER */}
        {cv.showOverview && (
        <div className="relative flex flex-col" style={{ minHeight: '100vh', pageBreakAfter: 'always', background: `linear-gradient(160deg, ${primaryColor} 0%, ${accentColor} 50%, ${accentColor}cc 100%)` }}>
          {cv.coverImage && <div className="absolute inset-0" style={{ backgroundImage: `url(${cv.coverImage})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.15 }} />}
        ...
        {/* CONTENT */}
        {cv.showPassengers && itin.passengerList.length > 0 && <div className="px-10 py-5" style={{ borderBottom: '1px solid #e2e8f0' }}><p className="text-[10px] uppercase tracking-[0.25em] mb-3" style={{ color: '#8599B4' }}>Travel Party</p><div className="flex flex-wrap gap-2">{itin.passengerList.map((p, i) => (<span key={i} className="px-3 py-1.5 rounded-lg text-sm" style={{ background: '#f0f5ff', color: primaryColor }}>{p.name}</span>))}</div></div>}

        {vdi.length > 0 && <div className="px-10 py-5" style={{ borderBottom: '1px solid #e2e8f0' }}><h2 className="text-xs font-bold uppercase tracking-[0.25em] mb-4" style={{ color: primaryColor }}>About Your Destinations</h2>{vdi.map((di, i) => (<div key={i} className="mb-4 last:mb-0"><h3 className="text-base font-bold mb-1" style={{ color: primaryColor }}>{di.name}</h3><p className="text-sm leading-relaxed" style={{ color: '#4b5563' }}>{di.description}</p></div>))}</div>}
        ...
        {days.map(([date, events], dayIdx) => {
          ...
          return (<div key={date}>
            <div className="px-10 py-3 flex items-center gap-4" style={{ background: dayIdx % 2 === 0 ? primaryColor : accentColor }}><div className="flex-shrink-0 w-10 h-10 rounded-lg flex flex-col items-center justify-center" style={{ background: 'rgba(255,255,255,0.15)' }}><span className="text-lg font-bold text-white leading-none">{dayNum}</span><span className="text-[7px] uppercase text-white/50">Day</span></div><div className="text-white"><p className="text-base font-bold leading-tight">{dateObj.toLocaleDateString('en-US', { weekday: 'long' })}, {dateObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p>{cities.length > 0 && <p className="text-[11px] opacity-50">{cities.join(', ')}</p>}</div></div>
            <div className="px-10 py-3">{events.map((ev, i) => { const color = TC[ev.type] || primaryColor; const bg = TBG[ev.type] || '#f0f5ff'; const cl = canEdit && ev.section && ev.itemId; return (<div key={i}>
              <div onClick={() => cl && handleClick(ev)} className={`flex gap-3 mb-2 last:mb-0 rounded-lg px-2 py-1.5 -mx-2 transition-all ${cl ? 'cursor-pointer hover:bg-blue-50/50 group' : ''}`}><div className="flex-shrink-0 w-16 pt-0.5 text-right">{ev.noTime ? <p className="text-[9px] font-medium italic no-print" style={{ color: '#ef4444' }}>Enter time</p> : ev.time && <p className="text-xs font-bold" style={{ color: primaryColor }}>{ev.time}</p>}<span className="inline-block mt-0.5 text-[7px] font-bold uppercase tracking-wider px-1 py-0.5 rounded" style={{ background: bg, color }}>{ev.type}</span></div><div className="flex flex-col items-center flex-shrink-0 pt-1"><div className="w-2 h-2 rounded-full border-[1.5px]" style={{ borderColor: color, background: 'white' }} />{i < events.length - 1 && <div className="w-px flex-1 mt-0.5" style={{ background: '#e2e8f0' }} />}</div><div className="flex-1 pb-1"><div className="flex items-center gap-2"><p className="font-bold text-xs" style={{ color: primaryColor }}>{ev.title}</p>{cl && <span className="no-print opacity-0 group-hover:opacity-100"><Icon n="edit" c="w-3 h-3" /></span>}</div><p className="text-[11px]" style={{ color: '#64748b' }}>{ev.sub}</p>{ev.detail && <p className="text-[10px]" style={{ color: '#94a3b8' }}>{ev.detail}</p>}</div></div>
              {ev.type === 'Check-in' && ev.hotelPhotos && ev.hotelPhotos.length > 0 && (
                <div className="ml-[76px] mb-3 flex gap-2 overflow-x-auto pb-1">
                  {ev.hotelPhotos.slice(0, 4).map((url, pi) => (
                    <img key={pi} src={url} alt={ev.title} className="w-28 h-20 rounded-lg object-cover flex-shrink-0" style={{ border: '1px solid #e2e8f0' }} />
                  ))}
                </div>
              )}
            </div>); })}</div>
        ...
        {cv.showDavening && (itin.davening || []).length > 0 && <div className="px-10 py-5" style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}><h2 className="text-xs font-bold uppercase tracking-[0.25em] mb-3" style={{ color: primaryColor }}>Davening / Minyan</h2>{(itin.davening || []).map((d, i) => (<div key={i} onClick={() => onEditItem?.('davening', d.id)} className={`p-3 rounded-lg bg-white mb-2 ${canEdit ? 'cursor-pointer hover:bg-blue-50/50 group' : ''}`} style={{ border: '1px solid #e2e8f0' }}><div className="flex items-center justify-between"><p className="font-bold text-xs" style={{ color: primaryColor }}>{d.location} <span className="font-normal" style={{ color: '#64748b' }}>&middot; {d.city}</span></p>{canEdit && <span className="no-print opacity-0 group-hover:opacity-100"><Icon n="edit" c="w-3 h-3" /></span>}</div><div className="grid grid-cols-3 gap-2 mt-2">{[['Shachris', d.shachris], ['Mincha', d.mincha], ['Maariv', d.mariv]].map(([l, v]) => (<div key={l} className="text-center p-1.5 rounded" style={{ background: '#f0f5ff' }}><p className="text-[8px] uppercase tracking-wider font-bold" style={{ color: '#8599B4' }}>{l}</p><p className="text-[11px] font-bold" style={{ color: v ? primaryColor : '#ef4444' }}>{v || 'Enter time'}</p></div>))}</div>{d.shabbos && <p className="text-[11px] mt-2 p-1.5 rounded" style={{ background: '#fefce8', color: '#92400e' }}><strong>Shabbos:</strong> {d.shabbos}</p>}</div>))}</div>}

        {cv.showMikvah && (itin.mikvah || []).length > 0 && <div className="px-10 py-5" style={{ borderTop: '1px solid #e2e8f0' }}><h2 className="text-xs font-bold uppercase tracking-[0.25em] mb-3" style={{ color: primaryColor }}>Mikvah</h2>{(itin.mikvah || []).map((m, i) => (<div key={i} onClick={() => onEditItem?.('mikvah', m.id)} className={`p-3 rounded-lg bg-white mb-2 ${canEdit ? 'cursor-pointer hover:bg-blue-50/50 group' : ''}`} style={{ border: '1px solid #e2e8f0' }}><div className="flex items-center justify-between"><p className="font-bold text-xs" style={{ color: primaryColor }}>{m.name} <span className="font-normal" style={{ color: '#64748b' }}>&middot; {m.city}</span></p>{canEdit && <span className="no-print opacity-0 group-hover:opacity-100"><Icon n="edit" c="w-3 h-3" /></span>}</div><p className="text-[11px]" style={{ color: '#94a3b8' }}>{m.address} &middot; {m.hours || 'Contact'}</p></div>))}</div>}

        {cv.showInsurance && itin.insurance.length > 0 && <div className="px-10 py-5" style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}><h2 className="text-xs font-bold uppercase tracking-[0.25em] mb-3" style={{ color: primaryColor }}>Insurance</h2>{itin.insurance.map((ins, i) => (<div key={i} onClick={() => onEditItem?.('insurance', ins.id)} className={`p-3 rounded-lg bg-white ${canEdit ? 'cursor-pointer hover:bg-blue-50/50 group' : ''}`} style={{ border: '1px solid #e2e8f0' }}><div className="flex items-center justify-between"><p className="font-bold text-xs" style={{ color: primaryColor }}>{ins.provider} <span className="font-normal" style={{ color: '#64748b' }}>&middot; {ins.coverage} &middot; {ins.policy}</span></p>{canEdit && <span className="no-print opacity-0 group-hover:opacity-100"><Icon n="edit" c="w-3 h-3" /></span>}</div></div>))}</div>}

        {cv.showNotes && itin.notes && <div className="px-10 py-5" style={{ borderTop: '1px solid #e2e8f0' }}><div className="p-4 rounded-lg" style={{ background: '#fefce8', border: '1px solid #fde68a' }}><p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: '#d97706' }}>Important Notes</p><p className="text-xs leading-relaxed" style={{ color: '#78350f' }}>{itin.notes}</p></div></div>}

        {cv.showContactInfo && <div className="px-10 py-4" style={{ background: '#f0f5ff', borderTop: '1px solid #D0E2FA' }}><div className="grid grid-cols-2 gap-4 text-[11px]"><div><p style={{ color: '#8599B4' }}>Travel Agency</p><p className="font-semibold" style={{ color: primaryColor }}>{agencyProfile.name}</p><p style={{ color: '#64748b' }}>{agencyProfile.phone}</p></div><div><p style={{ color: '#8599B4' }}>Your Advisor</p><p className="font-semibold" style={{ color: primaryColor }}>{itin.agent}</p><p style={{ color: '#64748b' }}>{agencyProfile.email}</p></div></div></div>}
        <div className="px-10 py-6 flex items-center justify-between" style={{ background: primaryColor }}><div className="flex items-center gap-3">{logo ? <img src={logo} alt="" className="h-8 object-contain brightness-0 invert" style={{ maxWidth: 120 }} /> : <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold" style={{ background: 'rgba(255,255,255,0.15)', color: 'white' }}>{agencyProfile.name.charAt(0)}</div>}<p className="text-sm font-bold text-white tracking-wider">{agencyProfile.name.toUpperCase()}</p></div><p className="text-[10px] text-white/30">Prepared for {itin.client}</p></div>
      </div>
    </div>
  );
}
