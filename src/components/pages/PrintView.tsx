...
interface Props { itin: Itinerary; agencyProfile: AgencyProfile; }
interface TLE { date: string; sk: string; type: string; title: string; sub: string; detail?: string; time?: string; city?: string; }
...
function buildTL(it: Itinerary): TLE[] {
  ...
  it.hotels.forEach((h) => { e.push({ date: h.checkIn, sk: h.checkIn + ' 14:00', type: 'Check-in', title: h.name, sub: `${h.city} \u00b7 ${h.roomType} \u00b7 ${h.rooms} room${h.rooms > 1 ? 's' : ''}`, detail: h.ref ? `Ref: ${h.ref}` : '', time: '2:00 PM', city: h.city }); e.push({ date: h.checkOut, sk: h.checkOut + ' 11:00', type: 'Check-out', title: h.name, sub: h.city, time: '11:00 AM', city: h.city }); });
  it.transport.forEach((t) => { const d = t.pickupDateTime.split('T')[0] || t.pickupDateTime.split(' ')[0]; e.push({ date: d, sk: t.pickupDateTime, type: 'Transfer', title: `${t.type}${t.carType ? ' \u2014 ' + t.carType : ''}`, sub: `${t.pickup} \u2192 ${t.dropoff}`, detail: t.provider || '', time: fmtTime12(t.pickupDateTime), city: t.dropoff }); });
  it.attractions.forEach((a) => { e.push({ date: a.date, sk: a.date + ' ' + (a.time || '09:00'), type: 'Activity', title: a.name, sub: `${a.city} \u00b7 ${a.ticketType}`, time: a.time ? fmtTime12(a.date + 'T' + a.time) : '', city: a.city }); });
...
export default function PrintView({ itin, agencyProfile }: Props) {
    ...
    <div>
      ...
      <div className="bg-white rounded-xl overflow-hidden shadow-lg" style={{ fontFamily: "'Georgia', serif" }}>
        ...
        <div className="relative flex flex-col" style={{ minHeight: '100vh', pageBreakAfter: 'always', background: 'linear-gradient(160deg, #071e45 0%, #0a3270 35%, #143F77 65%, #1a5298 100%)' }}>
          ...
          {/* Top bar: Logo + Agency */}
          <div className="relative px-12 pt-10 pb-4 flex items-center gap-4">
            {logo ? <img src={logo} alt="" className="h-12 object-contain" style={{ maxWidth: 160 }} /> : <div className="w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold" style={{ background: 'rgba(255,255,255,0.15)', color: 'white' }}>{agencyProfile.name.charAt(0)}</div>}
            <div><p className="text-sm font-semibold tracking-[0.3em] uppercase text-white/80">{agencyProfile.name}</p><p className="text-xs text-white/40 mt-0.5">{agencyProfile.address}</p></div>
          </div>

          {/* Center: Trip title — vertically centered */}
          <div className="relative flex-1 flex items-center px-12">
            <div className="max-w-2xl">
              <div className="w-16 h-1 rounded-full mb-8" style={{ background: 'rgba(208,226,250,0.4)' }} />
              <h1 className="text-6xl font-bold text-white leading-[1.1] mb-6" style={{ textShadow: '0 4px 30px rgba(0,0,0,0.3)' }}>{itin.title}</h1>
              <div className="flex items-center gap-4 text-lg text-white/70 flex-wrap">
                <span>{allDests}</span>
              ...
              <div className="flex items-center gap-6 mt-6">
                {[{ v: n, l: 'Nights' }, { v: itin.passengers, l: 'Travelers' }, { v: itin.flights.length, l: 'Flights' }, { v: itin.hotels.length, l: 'Hotels' }].map((s) => (
                  <div key={s.l} className="text-center">
                    ...
                    <p className="text-[10px] uppercase tracking-[0.2em] text-white/40 mt-1">{s.l}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Bottom: Prepared for bar */}
          <div className="relative px-12 pb-10 pt-6">
            <div className="flex items-end justify-between">
              <div>
                <p className="text-[10px] uppercase tracking-[0.3em] text-white/40 mb-1">Prepared for</p>
                <p className="text-3xl font-bold text-white">{itin.client}</p>
              </div>
              <div className="text-right">
                <p className="text-[10px] uppercase tracking-[0.3em] text-white/40 mb-1">Your Travel Advisor</p>
                <p className="text-xl font-semibold text-white">{itin.agent}</p>
                <p className="text-sm text-white/40 mt-1">{agencyProfile.phone} &middot; {agencyProfile.email}</p>
              </div>
            </div>
          </div>
        </div>
                ...
                {events.map((ev, i) => {
                  ...
                  return (
                    <div key={i} className="flex gap-3 mb-2 last:mb-0">
                      ...
                      <div className="flex-1 pb-1"><p className="font-bold text-xs" style={{ color: '#093168' }}>{ev.title}</p><p className="text-[11px]" style={{ color: '#64748b' }}>{ev.sub}</p>{ev.detail && <p className="text-[10px]" style={{ color: '#94a3b8' }}>{ev.detail}</p>}</div>
                    </div>
                  );
                })}
              </div>
        ...
        {/* Davening */}
        {(itin.davening || []).length > 0 && <div className="px-10 py-5" style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}><h2 className="text-xs font-bold uppercase tracking-[0.25em] mb-3" style={{ color: '#093168' }}>Davening / Minyan</h2>{(itin.davening || []).map((d, i) => (<div key={i} className="p-3 rounded-lg bg-white mb-2" style={{ border: '1px solid #e2e8f0' }}><p className="font-bold text-xs" style={{ color: '#093168' }}>{d.location} <span className="font-normal" style={{ color: '#64748b' }}>&middot; {d.city} &middot; {d.type}</span></p><div className="grid grid-cols-3 gap-2 mt-2">{[['Shachris', d.shachris], ['Mincha', d.mincha], ['Maariv', d.mariv]].map(([l, v]) => (<div key={l} className="text-center p-1.5 rounded" style={{ background: '#f0f5ff' }}><p className="text-[8px] uppercase tracking-wider font-bold" style={{ color: '#8599B4' }}>{l}</p><p className="text-[11px] font-bold" style={{ color: '#093168' }}>{v || '\u2014'}</p></div>))}</div>{d.shabbos && <p className="text-[11px] mt-2 p-1.5 rounded" style={{ background: '#fefce8', color: '#92400e' }}><strong>Shabbos:</strong> {d.shabbos}</p>}</div>))}</div>}

        {/* Mikvah */}
        {(itin.mikvah || []).length > 0 && <div className="px-10 py-5" style={{ borderTop: '1px solid #e2e8f0' }}><h2 className="text-xs font-bold uppercase tracking-[0.25em] mb-3" style={{ color: '#093168' }}>Mikvah</h2>{(itin.mikvah || []).map((m, i) => (<div key={i} className="p-3 rounded-lg bg-white mb-2" style={{ border: '1px solid #e2e8f0' }}><p className="font-bold text-xs" style={{ color: '#093168' }}>{m.name} <span className="font-normal" style={{ color: '#64748b' }}>&middot; {m.city} &middot; {m.gender}</span></p><p className="text-[11px]" style={{ color: '#94a3b8' }}>{m.address} &middot; {m.hours || 'Contact'}</p></div>))}</div>}

        {/* Insurance */}
        {itin.insurance.length > 0 && <div className="px-10 py-5" style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}><h2 className="text-xs font-bold uppercase tracking-[0.25em] mb-3" style={{ color: '#093168' }}>Insurance</h2>{itin.insurance.map((ins, i) => (<div key={i} className="p-3 rounded-lg bg-white" style={{ border: '1px solid #e2e8f0' }}><p className="font-bold text-xs" style={{ color: '#093168' }}>{ins.provider} <span className="font-normal" style={{ color: '#64748b' }}>&middot; {ins.coverage} &middot; {ins.policy}</span></p></div>))}</div>}
        ...
        <div className="px-10 py-6 flex items-center justify-between" style={{ background: '#093168' }}>
          <div className="flex items-center gap-3">
            {logo ? <img src={logo} alt="" className="h-8 object-contain brightness-0 invert" style={{ maxWidth: 120 }} /> : <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold" style={{ background: 'rgba(255,255,255,0.15)', color: 'white' }}>{agencyProfile.name.charAt(0)}</div>}
            <p className="text-sm font-bold text-white tracking-wider">{agencyProfile.name.toUpperCase()}</p>
          </div>
          <p className="text-[10px] text-white/30">Prepared for {itin.client}</p>
        </div>
      </div>
    </div>
  );
