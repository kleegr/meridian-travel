  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl overflow-hidden shadow-lg" style={{ fontFamily: ff }}>
        ...
        {cv.showOverview && (
          <div className="relative" style={{ background: isMinimal ? '#f8fafc' : `linear-gradient(135deg, ${pc} 0%, ${ac} 100%)` }}>
            ...
            <div className="relative px-8 py-6">
              ...
              <p className="text-xs italic leading-relaxed mb-3" style={{ color: isMinimal ? '#64748b' : 'rgba(255,255,255,0.7)' }}>{sectionIntro('overview', itin.client)}</p>
              <p className="text-sm mb-4" style={{ color: isMinimal ? '#64748b' : 'rgba(255,255,255,0.5)' }}>{allDests} | {fmtDate(itin.startDate)} - {fmtDate(itin.endDate)}</p>
              ...
              <div className="flex justify-between items-end mt-4 pt-4" style={{ borderTop: isMinimal ? '1px solid #e2e8f0' : '1px solid rgba(255,255,255,0.1)' }}>
                ...
                <div className="text-right"><p className="text-[8px] uppercase tracking-widest mb-0.5" style={{ color: isMinimal ? '#94a3b8' : 'rgba(255,255,255,0.4)' }}>Your Travel Advisor</p><p className="text-sm font-semibold" style={{ color: isMinimal ? pc : 'white' }}>{itin.agent}</p><p className="text-[10px]" style={{ color: isMinimal ? '#94a3b8' : 'rgba(255,255,255,0.4)' }}>{agencyProfile.phone}</p></div>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Passengers */}
        {cv.showPassengers && itin.passengerList.length > 0 && <div className="px-8 py-4" style={{ borderBottom: '1px solid #e2e8f0' }}><p className="text-[9px] italic mb-2" style={{ color: '#94a3b8' }}>{sectionIntro('passengers')}</p><div className="flex flex-wrap gap-1.5">{itin.passengerList.map((p, i) => (<span key={i} className="px-2.5 py-1 rounded text-xs" style={{ background: '#f0f5ff', color: pc }}>{p.name}</span>))}</div></div>}

        {/* Destination Info with hospitality intros */}
        {vdi.length > 0 && <div className="px-8 py-4" style={{ borderBottom: '1px solid #e2e8f0' }}><h2 className="text-[9px] font-bold uppercase tracking-widest mb-3" style={{ color: pc }}>About Your Destinations</h2>{vdi.map((di, i) => (<div key={i} className="mb-3 last:mb-0"><h3 className="text-sm font-bold mb-0.5" style={{ color: pc }}>{di.name}</h3><p className="text-[10px] italic mb-1" style={{ color: ac }}>{destinationIntro(di.name)}</p><p className="text-xs leading-relaxed" style={{ color: '#4b5563' }}>{di.description}</p></div>))}</div>}
        ...
        {days.map(([date, events], dayIdx) => {
          ...
          return (<div key={date}>
            ...
            {/* Day intro */}
            <div className="px-8 pt-2 pb-1"><p className="text-[10px] italic leading-relaxed" style={{ color: '#94a3b8' }}>{dayIntro(dayNum, cities, events.length)}</p></div>
            <div className="px-8 py-1">{events.map((ev, i) => { const color = TC[ev.type] || pc; const bg = TBG[ev.type] || '#f0f5ff'; const cl = canEdit && ev.section && ev.itemId; return (<div key={i} className="mb-2">
              {/* Hospitality intro line */}
              {ev.hospCopy && <p className="text-[9px] italic leading-relaxed ml-[70px] mb-0.5" style={{ color: '#94a3b8' }}>{ev.hospCopy}</p>}
              <div onClick={() => cl && handleClick(ev)} className={'flex gap-2.5 py-1 rounded-lg px-1.5 -mx-1.5 ' + (cl ? 'cursor-pointer hover:bg-blue-50/50 group' : '')}>
                ...
                <div className="flex-1 pb-0.5"><p className="font-bold text-[11px]" style={{ color: pc }}>{ev.title}</p><p className="text-[10px]" style={{ color: '#64748b' }}>{ev.sub}</p>{ev.detail && <p className="text-[9px]" style={{ color: '#94a3b8' }}>{ev.detail}</p>}</div>
              </div>
              {ev.type === 'Check-in' && ev.hotelPhotos && ev.hotelPhotos.length > 0 && (<div className="ml-[70px] mb-1 flex gap-1.5">{ev.hotelPhotos.slice(0, 3).map((url, pi) => (<img key={pi} src={url} alt="" className="w-20 h-14 rounded object-cover" style={{ border: '1px solid #e2e8f0' }} />))}</div>)}
            </div>); })}</div>
          </div>);
        })}

        {/* Davening */}
        {cv.showDavening && (itin.davening || []).length > 0 && <div className="px-8 py-4" style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}><h2 className="text-[9px] font-bold uppercase tracking-widest mb-1" style={{ color: pc }}>Davening / Minyan</h2><p className="text-[9px] italic mb-2" style={{ color: '#94a3b8' }}>{sectionIntro('davening')}</p>{(itin.davening || []).map((d, i) => (<div key={i} onClick={() => onEditItem?.('davening', d.id)} className={'p-2 rounded bg-white mb-1.5 ' + (canEdit ? 'cursor-pointer hover:bg-blue-50/50' : '')} style={{ border: '1px solid #e2e8f0' }}><p className="font-bold text-[10px]" style={{ color: pc }}>{d.location} <span className="font-normal" style={{ color: '#64748b' }}>| {d.city}</span></p><div className="flex gap-4 mt-1 text-[9px]">{[['Shachris', d.shachris], ['Mincha', d.mincha], ['Maariv', d.mariv]].map(([l, v]) => (<span key={l as string} style={{ color: v ? pc : '#ef4444' }}><strong>{l}:</strong> {(v as string) || 'TBD'}</span>))}</div>{d.shabbos && <p className="text-[9px] mt-1" style={{ color: '#92400e' }}>Shabbos: {d.shabbos}</p>}</div>))}</div>}

        {/* Mikvah */}
        {cv.showMikvah && (itin.mikvah || []).length > 0 && <div className="px-8 py-4" style={{ borderTop: '1px solid #e2e8f0' }}><h2 className="text-[9px] font-bold uppercase tracking-widest mb-1" style={{ color: pc }}>Mikvah</h2><p className="text-[9px] italic mb-2" style={{ color: '#94a3b8' }}>{sectionIntro('mikvah')}</p>{(itin.mikvah || []).map((m, i) => (<div key={i} onClick={() => onEditItem?.('mikvah', m.id)} className={'p-2 rounded bg-white mb-1.5 ' + (canEdit ? 'cursor-pointer hover:bg-blue-50/50' : '')} style={{ border: '1px solid #e2e8f0' }}><p className="font-bold text-[10px]" style={{ color: pc }}>{m.name} <span className="font-normal" style={{ color: '#64748b' }}>| {m.city}</span></p><p className="text-[9px]" style={{ color: '#94a3b8' }}>{m.address} | {m.hours || 'Contact for hours'}</p></div>))}</div>}

        {/* Insurance */}
        {cv.showInsurance && itin.insurance.length > 0 && <div className="px-8 py-4" style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}><h2 className="text-[9px] font-bold uppercase tracking-widest mb-1" style={{ color: pc }}>Travel Protection</h2><p className="text-[9px] italic mb-2" style={{ color: '#94a3b8' }}>{sectionIntro('insurance')}</p>{itin.insurance.map((ins, i) => (<p key={i} className="text-[10px] mb-1" style={{ color: '#4b5563' }}><strong style={{ color: pc }}>{ins.provider}</strong> | {ins.coverage} | {ins.policy}</p>))}</div>}

        {/* Notes */}
        {cv.showNotes && itin.notes && <div className="px-8 py-4" style={{ borderTop: '1px solid #e2e8f0' }}><div className="p-3 rounded" style={{ background: '#fefce8', border: '1px solid #fde68a' }}><p className="text-[8px] font-bold uppercase tracking-wider mb-0.5" style={{ color: '#d97706' }}>From Your Advisor</p><p className="text-[9px] italic mb-1" style={{ color: '#b45309' }}>{sectionIntro('notes')}</p><p className="text-[10px] leading-relaxed" style={{ color: '#78350f' }}>{itin.notes}</p></div></div>}

        {/* Contact Footer */}
        {cv.showContactInfo && <div className="px-8 py-4" style={{ background: '#f0f5ff', borderTop: '1px solid #D0E2FA' }}><p className="text-[9px] italic mb-2" style={{ color: '#94a3b8' }}>{sectionIntro('contact', itin.client)}</p><div className="flex items-center justify-between text-[9px]"><div><span className="font-semibold" style={{ color: pc }}>{agencyProfile.name}</span> <span style={{ color: '#64748b' }}>{agencyProfile.phone}</span></div><div style={{ color: '#64748b' }}>{itin.agent} | {agencyProfile.email}</div></div></div>}
        <div className="px-8 py-3 flex items-center justify-between" style={{ background: pc }}>{logo ? <img src={logo} alt="" className="h-5 object-contain brightness-0 invert" /> : <span className="text-[10px] font-bold text-white tracking-wider">{agencyProfile.name.toUpperCase()}</span>}<p className="text-[8px] text-white/30">Prepared with care for {itin.client}</p></div>
      </div>
    </div>
  );
}
