...
const SECTIONS = [
  ...
  ['Pipelines & Stages', 'Create pipelines with stages for your board columns', 'kanban'],
...
export default function Settings(props: SettingsProps) {
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => { props.setAgencyProfile({ ...props.agencyProfile, logo: reader.result as string }); };
      ...
      <div className="space-y-5">
        ...
        {activeSection === 'Agency Profile' && (
          <div className="bg-white rounded-xl border p-6 shadow-sm" style={{ borderColor: GHL.border }}>
            ...
            <p className="text-sm mb-6" style={{ color: GHL.muted }}>This information appears on all itineraries and printed documents</p>

            {/* Logo Upload */}
            <div className="mb-6">
              <label className="block text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: GHL.muted }}>Company Logo</label>
              <div className="flex items-center gap-4">
                <div className="w-24 h-24 rounded-xl border-2 border-dashed flex items-center justify-center overflow-hidden cursor-pointer hover:border-blue-300 transition-colors" style={{ borderColor: props.agencyProfile.logo ? GHL.accent : GHL.border, background: props.agencyProfile.logo ? 'white' : GHL.bg }} onClick={() => fileRef.current?.click()}>
                  {props.agencyProfile.logo ? (
                    <img src={props.agencyProfile.logo} alt="Logo" className="w-full h-full object-contain p-2" />
                  ) : (
                    <div className="text-center"><Icon n="plus" c="w-6 h-6 mx-auto" /><p className="text-[9px] mt-1" style={{ color: GHL.muted }}>Upload</p></div>
                  )}
                </div>
                <div>
                  <button onClick={() => fileRef.current?.click()} className="text-sm font-medium px-4 py-2 rounded-lg border hover:bg-gray-50" style={{ borderColor: GHL.border, color: GHL.accent }}>
                    {props.agencyProfile.logo ? 'Change Logo' : 'Upload Logo'}
                  </button>
                  {props.agencyProfile.logo && <button onClick={() => props.setAgencyProfile({ ...props.agencyProfile, logo: '' })} className="text-sm ml-2 px-3 py-2 rounded-lg text-red-400 hover:text-red-600 hover:bg-red-50">Remove</button>}
                  <p className="text-[10px] mt-1.5" style={{ color: GHL.muted }}>PNG, JPG or SVG. Max 2MB. Shows on itineraries and PDF.</p>
                </div>
            ...
            <p className="text-xs mt-4" style={{ color: GHL.muted }}>Changes are applied immediately to all itineraries and the client PDF.</p>
          </div>
        )}
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-xl border p-6 shadow-sm" style={{ borderColor: GHL.border }}>
              <div className="flex items-center justify-between mb-4">
                <div><h3 className="font-semibold text-lg" style={{ color: GHL.text }}>Pipelines &amp; Stages</h3><p className="text-sm" style={{ color: GHL.muted }}>Active pipeline stages become board columns</p></div>
              ...
              <div className="flex gap-2 flex-wrap mb-4">{props.pipelines.map((p) => (<button key={p.id} onClick={() => props.setActivePipelineId(p.id)} className="px-4 py-2 rounded-lg text-sm font-medium transition-colors" style={props.activePipelineId === p.id ? { background: GHL.accent, color: 'white' } : { background: GHL.bg, color: GHL.muted, border: `1px solid ${GHL.border}` }}>{p.name} ({p.stages.length})</button>))}</div>

              {activePipeline && <div>
                <div className="flex items-center justify-between mb-3"><p className="text-sm font-semibold" style={{ color: GHL.text }}>{activePipeline.name} &mdash; Stages</p>{props.pipelines.length > 1 && <button onClick={() => deletePipeline(activePipeline.id)} className="text-xs text-red-400 hover:text-red-600">Delete Pipeline</button>}</div>
                <div className="space-y-2">{activePipeline.stages.map((stage, idx) => (<div key={idx} draggable onDragStart={() => handleDragStart(idx)} onDragOver={(e) => handleDragOver(e, idx)} onDragEnd={handleDragEnd} className="flex items-center gap-3 p-3 rounded-lg border hover:bg-blue-50/30 cursor-grab" style={{ borderColor: GHL.border, opacity: dragIdx === idx ? 0.5 : 1 }}><span style={{ color: GHL.muted }}><Icon n="grip" c="w-4 h-4" /></span><span className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ background: GHL.accent }}>{idx + 1}</span><span className="flex-1 font-medium text-sm" style={{ color: GHL.text }}>{stage}</span><button onClick={() => deleteStage(idx)} className="p-1 rounded hover:bg-red-50 text-gray-300 hover:text-red-500"><Icon n="trash" c="w-3.5 h-3.5" /></button></div>))}</div>
                <div className="flex gap-2 mt-4 pt-4 border-t" style={{ borderColor: GHL.border }}><input value={newStage} onChange={(e) => setNewStage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addStage()} placeholder="Add stage..." className="flex-1 px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-200" style={{ borderColor: GHL.border }} /><button onClick={addStage} className="px-4 py-2.5 text-sm font-semibold text-white rounded-lg" style={{ background: GHL.accent }}>Add</button></div>
              </div>}
            </div>
          </div>
        )}
        ...
        {activeSection === 'Custom Fields' && (
          <div className="bg-white rounded-xl border p-6 shadow-sm" style={{ borderColor: GHL.border }}>
            ...
            <div className="flex gap-2"><input value={newItem} onChange={(e) => setNewItem(e.target.value)} placeholder="Field name..." className="flex-1 px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-200" style={{ borderColor: GHL.border }} /><select id="cf-module" className="px-3 py-2.5 border rounded-lg text-sm bg-white" style={{ borderColor: GHL.border }}><option>Itinerary</option><option>Flight</option><option>Hotel</option><option>Transport</option></select><select id="cf-type" className="px-3 py-2.5 border rounded-lg text-sm bg-white" style={{ borderColor: GHL.border }}><option>Text</option><option>Number</option><option>Date</option><option>Dropdown</option></select><button onClick={() => { if (!newItem.trim()) return; const mod = (document.getElementById('cf-module') as HTMLSelectElement).value; const tp = (document.getElementById('cf-type') as HTMLSelectElement).value; props.setCustomFields([...props.customFields, { id: uid(), name: newItem.trim(), module: mod, type: tp }]); setNewItem(''); }} className="px-4 py-2.5 text-sm font-semibold text-white rounded-lg" style={{ background: GHL.accent }}>Add</button></div>
          </div>
        )}
      </div>
    );
