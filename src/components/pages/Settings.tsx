...
import type { AgencyProfile, CustomField, Pipeline, ChecklistTemplate, StageColor } from '@/lib/types';
...
export default function Settings(props: SettingsProps) {
  ...
  // Stage color helpers
  const getStageColor = (stage: string): { color: string; bg: string; dot: string } => {
    const sc = activePipeline?.stageColors?.find((s) => s.stage === stage);
    if (sc) return { color: sc.color, bg: sc.bg, dot: sc.color };
    const preset = STAGE_COLOR_PRESETS.find((p) => p.label === 'Gray');
    return preset || STAGE_COLOR_PRESETS[0];
  };
  const setStageColor = (stage: string, preset: typeof STAGE_COLOR_PRESETS[0]) => {
    if (!activePipeline) return;
    const existing = (activePipeline.stageColors || []).filter((s) => s.stage !== stage);
    const newColors: StageColor[] = [...existing, { stage, color: preset.color, bg: preset.bg }];
    props.setPipelines(props.pipelines.map((p) => p.id === activePipeline.id ? { ...p, stageColors: newColors } : p));
  ...
  if (activeSection) {
    return (
      <div className="space-y-5">
        <button onClick={() => { setActiveSection(null); setNewItem(''); setEditingTplId(null); setColorPickerStage(null); }} className="inline-flex items-center gap-2 text-sm hover:opacity-80" style={{ color: GHL.muted }}><Icon n="back" c="w-4 h-4" /> Back to Settings</button>
        ...
        {/* ===== PIPELINES & STAGES WITH COLOR PICKER ===== */}
          ...
          {activePipeline && <div>
            ...
            <div className="space-y-2">
              {activePipeline.stages.map((stage, idx) => {
                ...
                const isPickerOpen = colorPickerStage === stage;
                return (
                  <div key={idx}>
                    <div draggable onDragStart={() => handleDragStart(idx)} onDragOver={(e) => handleDragOver(e, idx)} onDragEnd={handleDragEnd} className="flex items-center gap-3 p-3 rounded-lg border cursor-grab" style={{ borderColor: GHL.border, opacity: dragIdx === idx ? 0.5 : 1 }}>
                      <span style={{ color: GHL.muted }}><Icon n="grip" c="w-4 h-4" /></span>
                      {/* Color dot — click to open picker */}
                      <button onClick={() => setColorPickerStage(isPickerOpen ? null : stage)} className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 border-2 transition-all hover:scale-110" style={{ background: sc.bg, borderColor: sc.color, color: sc.color }} title="Change color">{idx + 1}</button>
                      <span className="flex-1 font-medium text-sm" style={{ color: GHL.text }}>{stage}</span>
                      <span className="text-[9px] font-semibold px-2 py-0.5 rounded" style={{ background: sc.bg, color: sc.color }}>{stage}</span>
                      <button onClick={() => deleteStage(idx)} className="p-1 rounded hover:bg-red-50 text-gray-300 hover:text-red-500"><Icon n="trash" c="w-3.5 h-3.5" /></button>
                    </div>
                    {/* Color picker dropdown */}
                    {isPickerOpen && <div className="ml-12 mt-1 mb-2 p-3 rounded-lg border" style={{ borderColor: GHL.border, background: '#fafbfc' }}>
                      <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: GHL.muted }}>Pick color for "{stage}"</p>
                      <div className="flex flex-wrap gap-2">
                        {STAGE_COLOR_PRESETS.map((preset) => (
                          <button key={preset.label} onClick={() => setStageColor(stage, preset)} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold border transition-all hover:scale-105" style={{ background: preset.bg, borderColor: preset.color + '40', color: preset.color }}>
                            <span className="w-3 h-3 rounded-full" style={{ background: preset.dot }} />
                            {preset.label}
                          </button>
                        ))}
                      </div>
        ...
        {activeSection === 'Checklist Templates' && <div className="space-y-5"><div className="bg-white rounded-xl border p-6 shadow-sm" style={{ borderColor: GHL.border }}><div className="flex items-center justify-between mb-4"><div><h3 className="font-semibold text-lg" style={{ color: GHL.text }}>Checklist Templates</h3><p className="text-sm" style={{ color: GHL.muted }}>Create named checklists for itineraries.</p></div></div><div className="space-y-2 mb-4">{props.checklistTemplates.map((tpl) => (<div key={tpl.id} className="rounded-lg border overflow-hidden" style={{ borderColor: editingTplId === tpl.id ? GHL.accent : GHL.border }}><div className="flex items-center justify-between px-4 py-3 cursor-pointer" style={{ background: editingTplId === tpl.id ? '#f0f5ff' : GHL.bg }} onClick={() => setEditingTplId(editingTplId === tpl.id ? null : tpl.id)}><div className="flex items-center gap-3"><span className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-white" style={{ background: GHL.accent }}>{tpl.items.length}</span><div><p className="font-semibold text-sm" style={{ color: GHL.text }}>{tpl.name}</p><p className="text-[10px]" style={{ color: GHL.muted }}>{tpl.items.length} items</p></div></div><div className="flex items-center gap-2"><button onClick={(e) => { e.stopPropagation(); deleteTemplate(tpl.id); }} className="p-1 rounded hover:bg-red-50 text-gray-300 hover:text-red-500"><Icon n="trash" c="w-3.5 h-3.5" /></button><Icon n={editingTplId === tpl.id ? 'chevronDown' : 'chevronRight'} c="w-4 h-4" /></div></div>{editingTplId === tpl.id && <div className="px-4 py-4 border-t" style={{ borderColor: GHL.border }}><div className="mb-4"><label className="block text-xs font-semibold uppercase tracking-wider mb-1" style={{ color: GHL.muted }}>Template Name</label><input value={tpl.name} onChange={(e) => renameTpl(tpl.id, e.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm" style={{ borderColor: GHL.border }} /></div><div className="space-y-1.5 mb-3">{tpl.items.map((item, idx) => (<div key={idx} className="flex items-center gap-2 py-1.5 px-3 rounded-lg" style={{ background: GHL.bg }}><span className="text-xs font-bold w-5 text-center" style={{ color: GHL.muted }}>{idx + 1}</span><span className="flex-1 text-sm" style={{ color: GHL.text }}>{item}</span><button onClick={() => removeTplItem(idx)} className="p-0.5 rounded hover:bg-red-50 text-gray-300 hover:text-red-500"><Icon n="trash" c="w-3 h-3" /></button></div>))}{tpl.items.length === 0 && <p className="text-xs text-center py-3" style={{ color: GHL.muted }}>No items yet.</p>}</div><div className="flex gap-2"><input value={newTplItem} onChange={(e) => setNewTplItem(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addTplItem()} placeholder="Add checklist item..." className="flex-1 px-3 py-2 border rounded-lg text-sm" style={{ borderColor: GHL.border }} /><button onClick={addTplItem} className="px-4 py-2 text-sm font-semibold text-white rounded-lg" style={{ background: GHL.accent }}>Add</button></div></div>}</div>))}</div><div className="flex gap-2 pt-4 border-t" style={{ borderColor: GHL.border }}><input value={newTplName} onChange={(e) => setNewTplName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addTemplate()} placeholder="New template name..." className="flex-1 px-3 py-2.5 border rounded-lg text-sm" style={{ borderColor: GHL.border }} /><button onClick={addTemplate} className="px-5 py-2.5 text-sm font-semibold text-white rounded-lg" style={{ background: GHL.accent }}>Create Template</button></div></div></div>}
