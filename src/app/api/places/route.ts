...
// Requires GOOGLE_PLACES_API_KEY env var (server-side, NOT the public one)
...
export async function GET(req: NextRequest) {
  ...
  try {
    // Action: search — find a hotel by name + city
    if (action === 'search') {
    ...
    // Action: details — get full details for a place_id
    if (action === 'details') {
    ...
    // Action: photo — proxy a photo (avoids exposing API key to client)
    if (action === 'photo') {
    ...
    return NextResponse.json({ error: 'Invalid action. Use: search, details, photo' }, { status: 400 });
  } catch (err: any) {
