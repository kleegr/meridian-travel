...
import { Dashboard, ItineraryList, Financials, Travelers, Pipelines, Settings } from '@/components/pages';
...
export default function App() {
  ...
  const toggleWidget = (id: string) => {
    setDashWidgets((prev) => prev.map((w) => (w.id === id ? { ...w, enabled: !w.enabled } : w)));
  };
  ...
  const pageTitle =
    ...
    : page === 'detail' && selectedItin ? selectedItin.title
    : '';
    ...
    <div className="min-h-screen flex" style={{ background: GHL.bg, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <main className="flex-1 md:ml-60 min-h-screen flex flex-col">
        <Header
          ...
          onSelectItinerary={handleSelect}
          onNavigate={handleNavigate}
        ...
        <div className="flex-1 p-4 md:p-8 overflow-auto pb-20 md:pb-8">
          {page === 'dashboard' && (
            <Dashboard itineraries={itineraries} widgets={dashWidgets} onToggleWidget={toggleWidget} />
          )}
          {page === 'itineraries' && (
            <ItineraryList itineraries={itineraries} onSelect={handleSelect} onCreate={() => setShowNewModal(true)} />
          )}
          {page === 'travelers' && (
            <Travelers itineraries={itineraries} onSelectItinerary={handleSelect} />
          )}
          {page === 'pipelines' && (
            <Pipelines pipelines={pipelines} onUpdate={setPipelines} />
          )}
          {page === 'financials' && (
            <Financials itineraries={itineraries} />
          )}
          {page === 'detail' && selectedItin && (
            <ItineraryDetail itin={selectedItin} onBack={handleBack} onUpdate={handleUpdate} />
          )}
          {page === 'settings' && (
            <Settings
              bookingSources={bookingSources}
              setBookingSources={setBookingSources}
              suppliers={suppliers}
      ...
      {showNewModal && (
        <NewItineraryModal onClose={() => setShowNewModal(false)} onCreate={handleCreate} />
      )}
    </div>
  );
}
