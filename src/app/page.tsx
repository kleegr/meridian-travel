...
// Optional SSO - only import if module exists
...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsed.activeLocation) setLocationId(parsed.activeLocation);
    } catch {}
  }, [SSO]);

  // Load from Supabase if SSO provides locationId
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (itinRes.status === 'fulfilled' && itinRes.value.data?.success) {
          ...
          // If DB returns empty, KEEP sample data (don't clear)
        }
  ...
  // Save helpers (only work with SSO)
  const saveItinerary = useCallback(async (itin: Itinerary) => {
    if (!locationId || !axios) return;
    try { await axios.post('/api/itineraries', { locationId, itinerary: itin }); } catch {}
  }, [locationId]);

  const deleteItineraryDB = useCallback(async (itineraryId: number) => {
    if (!locationId || !axios) return;
    try { await axios.delete('/api/itineraries', { data: { locationId, itineraryId } }); } catch {}
  }, [locationId]);
  ...
  const handleUpdate = useCallback((updated: Itinerary) => {
    setItineraries((prev) => prev.map((i) => (i.id === updated.id ? updated : i))); saveItinerary(updated);
  }, [saveItinerary]);

  const handleUpdateStatus = useCallback((id: number, newStatus: string) => {
    setItineraries((prev) => { const next = prev.map((i) => (i.id === id ? { ...i, status: newStatus } : i)); const u = next.find((i) => i.id === id); if (u) saveItinerary(u); return next; });
  }, [saveItinerary]);

  const handleDelete = useCallback((id: number) => {
    setItineraries((prev) => prev.filter((i) => i.id !== id)); deleteItineraryDB(id);
    if (selectedId === id) { setPage('itineraries'); setSelectedId(null); }
  }, [selectedId, deleteItineraryDB]);
  ...
  const handleCreateFromPackage = useCallback((pkg: PackageTemplate) => {
    ...
    const itin: Itinerary = {
      ...
      destinationInfo: [], packageTemplateId: pkg.id, tripType: pkg.tripType,
